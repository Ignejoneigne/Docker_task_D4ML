...
# Update the system and install necessary packages, specifying the Apache version
sudo apt-get update
...
sudo cat <<EOL > my-apache-container/Dockerfile
...
# Copy the "CARD.jpg" image to the directory where the Dockerfile is located
...
aws ec2 authorize-security-group-ingress --group-id $SECURITY_GROUP_ID --protocol tcp --port 15100 --source $MY_PUBLIC_IP/32
